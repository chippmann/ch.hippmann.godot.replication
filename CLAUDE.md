...
- **Scenario classes are loaded reflectively on BOTH classpaths**: by Godot in the peer process (full godot-api available) AND by the orchestrator's test JVM (godot-api NOT available — it only reads `scenePath` via reflection). Field initializers in scenario classes that reference `godot.core.*` types (Vector3, Transform3D, etc.) crash class loading on the orchestrator side with `NoClassDefFoundError`. Construct godot types lazily inside `runAsServer`/`runAsClient`, never as `val` field initializers on the scenario class.
